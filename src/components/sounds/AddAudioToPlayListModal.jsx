...
const AddAudioToPlayListModal = ({ setShowModal, showModal, audio }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("/members/Create_user_audio_list/", {
        list_name: title
      });
      if (res.status === 200 || res.status === 201) {
        const secondRes = await axios.put(
          `/members/Add_audio_to_user_list/${res?.data?.object?.id}/`,
          {
            audio_id: audio?.id
          }
        );
  ...
  const handleAddAudioToList = async () => {
    try {
      ...
      const res = await axios.put(
        `/members/Add_audio_to_user_list/${selectedList}/`,
        {
          audio_id: audio?.id
        }
      );
